...
EmulateJavaRandom::EmulateJavaRandom(std::int64_t seed) {
	setSeed(seed);
}

void EmulateJavaRandom::setRawSeed(std::int64_t s) {
	eng_.seed(s);
}
...
std::int32_t EmulateJavaRandom::nextInt() {
	return next(32);
}
...
std::uint64_t EmulateJavaRandom::next48() {
	return eng_();
}
